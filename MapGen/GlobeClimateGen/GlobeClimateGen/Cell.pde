...
class Cell implements Comparable<Cell> {
  ...
  int size;
  int borders;
  int distToSea = Integer.MAX_VALUE;
  ...
  void slopeNeighbors(float target, PriorityQueue<Cell> active, Grid grid) {
    int lowestDistToSea = Integer.MAX_VALUE;
    for (int i = -1; i < 2; i++)
      for (int j = -1; j < 2; j++) {
        ...
        if (cel.distToSea < lowestDistToSea) {
          lowestDistToSea = cel.distToSea;
        }
      }
    distToSea = lowestDistToSea + 1;
  }

  void continentalDist(ArrayList<Cell> active, Grid grid) {
    for (int i = -1; i < 2; i++)
      for (int j = -1; j < 2; j++) {
        if (abs(i) + abs(j) == 1) {
          Cell cel = grid.getCell(xPos + i, yPos + j);
          if (cel.distToSea > distToSea + 1) {
            cel.distToSea = distToSea + 1;
            active.add(cel);
          }
        }
      }
  ...
  void calculateTemperature(Grid grid) {
    float temp = 0;
    if (water == false) {
      temp += pow(map(abs((grdHeight/2) - yPos) * 2, 0, grdHeight, 0.9, 0.1), 1);
    } else {
  ...
  void getMoisture(Grid grid) {
    ...
    if (!water)
      if (abs(windDir.x) != 0) {
        ...
        for (int i = 0; i < grid.gridWidth/2 * lat; i++) {
          ...
          if (cel.size > 0) {
            ...
            if (cel.water == false) {
              ...
              avgElevation += landElevation;
              landCount++;
              if (landElevation > 0.7) {
                avgExtremeElevation += landElevation;
                extremeCount++;
              }
              if (landElevation > 0.9) {
                break;
              }
            }
            count++;
        ...
        moisture += pow(0.5 * waterPercentage + 0.25 * map(avgTemperature, 0, 1, 0.4, 0.8) + 0.25 * (1 - avgElevation), 3);
        //if (water == true) {
        ...
        lat = abs((grid.gridHeight/2) - (yPos + jitter ));
        lat = map(lat, 0, grid.gridHeight/2, 0, 90);
        if (lat <= 15) {
        ...
        if (lat >= 60 && lat <= 80) {
          if (lat >= 75) {
            lat = map(lat, 70, 80, 1, 0);
          } else {
            lat = map(lat, 60, 70, 0, 1);
          }
      ...
      for (int i = -grid.gridHeight/25; i < grid.gridHeight/25; i++) {
        for (int j = -grid.gridHeight/25; j < grid.gridHeight/25; j++) {
          if (!(i == 0 && j == 0)) {
            Cell cel = grid.getCell(xPos + i, yPos + j);
            if (cel.size > 0) {
              moistTotal += cel.nextMoisture;
              count++;
            }
          }
        }
      }
      nextMoisture = moistTotal/count;
    }

    lat = abs((grid.gridHeight/2) - yPos) * 2;
    lat = (int) map(lat, 0, grid.gridHeight, 0, 90);
    if (lat <= 15) {
      lat = map(lat, 0, 10, 1, 0);
      nextMoisture += (nextMoisture + 0.05) * 0.4 * lat;
    }
    if (lat >= 20 && lat <= 40) {
      if (lat >= 30) {
        lat = map(lat, 30, 40, 1, 0);
    ...
    if (lat >= 60) {
      ...
      nextMoisture += (nextMoisture + 0.05) * 0.2 * lat;
    }


    if (nextMoisture > 1)
      nextMoisture = 1;
    else if (nextMoisture < 0)
      nextMoisture = 0;
  }
